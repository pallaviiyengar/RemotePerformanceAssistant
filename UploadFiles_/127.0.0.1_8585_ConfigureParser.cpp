...
ConfigParseToConsole::~ConfigParseToConsole()
{
  ...
  delete pSpecialKeyword;
  delete pPushTypeKeyword;
  delete pTypeKeyword;
...
Parser* ConfigParseToConsole::Build()
{
  try
  {
	...
	// configure to detect and act on type keywords like  "class", "struct", "enum"
	pTypeKeyword = new TypeKeyword;
	pPushTypeKeyword = new PushTypeKeyword(pRepo);
	pTypeKeyword->addAction(pPushTypeKeyword);
	pParser->addRule(pTypeKeyword);
	...
	// configure to detect and act on special keywords like  "for", "while", "switch", "if", "catch"
...

std::vector<element> ConfigParseToConsole::getAnalysedScopes(){
	return pRepo->getanalysedScopes();
}
