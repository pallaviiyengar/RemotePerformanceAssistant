...
std::vector<std::string> list = fm.searchFiles(dir, "*.*", false);
...
//Class that provides applicatopn specific operations on files
class FileManager{
private:
...
public:
	...
	std::vector<std::string> searchFiles(std::string path, std::string pattern, bool listRecursively);
};
#endif